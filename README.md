...
| ✅ Committed | ❌ Excluded (in .gitignore) |
|-------------|--------------------------|
| All Python source files | `data/**/*.pdb` (auto-downloaded) |
| `dashboard/` (index.html, app.js, styles.css) | `data/**/*_centrality.csv` (computed) |
| `requirements.txt` | `data/**/*_plddt_per_residue.csv` (computed) |
| `README.md` | `data/**/*_layer0.json` (per-variant cache) |
| `data/*/VCP_clinvar_variants.json` (seed data) | `data/**/*_literature.json` (cached) |
| `data/VCP/VCP_conservation.csv` (precomputed) | `reports/` (generated reports) |
| | `validation_*.txt`, `run5_output.txt` (dev logs) |
| | `__pycache__/` (bytecode) |
| | `.claude/` (local IDE config) |
| | Debug scripts (`analyze_errors.py`, `check_*.py`, etc.) |
| | Dashboard backups (`*-backup.*`, `*-new.*`) |
| | Stale docs (`FOLDX_*.md`, `CONTEXT.md`, etc.) |

### Step-by-Step Git Push

#### Step 1 — Create the GitHub repo
Go to **github.com → New repository**:
- Name: `digital-patient-twin`
- Visibility: Public or Private
- **Leave "Initialize repository" unchecked** (we have local files already)
- Click **Create repository**
- Copy the repo URL (e.g., `https://github.com/kanishka-atrai/digital-patient-twin.git`)
...
```powershell
cd "C:\Users\kanis\.gemini\antigravity\scratch\digital-patient-twin"
...
git add variant_utils.py
git add pipeline_router.py
git add fetch_structure.py
git add plddt_audit.py
git add grantham.py
git add conservation.py
git add literature_engine.py
git add idr_analysis.py
git add clinical_brief.py
git add run_pipeline.py
git add clinvar_transformer.py
git add position_mapper.py
...
# Stage curated seed data (these are inputs, not outputs)
git add "data/VCP/VCP_clinvar_variants.json"
git add "data/VCP/VCP_conservation.csv"
git add "data/LMNA/LMNA_clinvar_variants.json"
git add "data/CFTR/CFTR_clinvar_variants.json"
git add "data/HEXA/HEXA_clinvar_variants.json"
git add "data/MECP2/MECP2_clinvar_variants.json"
```
...
You should see ONLY: source `.py` files, `dashboard/` files, `requirements.txt`, `README.md`, `.gitignore`, and the five `*_clinvar_variants.json` / `VCP_conservation.csv` files.

If anything unexpected appears (PDB files, CSV files, validation logs), it means `.gitignore` didn't catch it — remove it with:
```powershell
...
```powershell
git commit -m "feat: Digital Patient Twin v1.0 — structure-based variant interpretation platform"

# Connect to GitHub (replace with your actual URL)
git remote add origin https://github.com/kanishka-atrai/digital-patient-twin.git
...
├── data/
│   ├── CFTR/
│   │   └── CFTR_clinvar_variants.json
│   ├── HEXA/
│   │   └── HEXA_clinvar_variants.json
│   ├── LMNA/
│   │   └── LMNA_clinvar_variants.json
│   ├── MECP2/
│   │   └── MECP2_clinvar_variants.json
│   └── VCP/
│       ├── VCP_clinvar_variants.json
│       └── VCP_conservation.csv
├── .gitignore
├── README.md
├── requirements.txt
├── server.py
...
No PDB files, no CSVs except conservation, no reports, no validation logs.

### For Future Updates

```powershell
# After making changes:
git add server.py build_graph.py   # add only changed files explicitly
git commit -m "fix: correct l1_result NameError in _compute_layer0_and_report"
git push
```
